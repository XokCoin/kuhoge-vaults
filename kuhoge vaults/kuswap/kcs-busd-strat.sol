...
// File: contracts/BIFI/interfaces/panbusd/IMasterChef.sol
...
// File: contracts/BIFI/strategies/busd/StrategybusdLP.sol
...
contract StrategybusdLP is StratManager, FeeManager, GasThrottler {
    ...
    // Routes
    address[] public busdToWbnbRoute = [busd, wbnb];
    address[] public busdToLp0Route;
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(busd).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWbnb, 0, busdToWbnbRoute, address(this), now);
    ...
    function addLiquidity() internal {
        uint256 busdHalf = IERC20(busd).balanceOf(address(this)).div(2);

        if (lpToken0 != busd) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(busdHalf, 0, busdToLp0Route, address(this), now);
        }

        if (lpToken1 != busd) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(busdHalf, 0, busdToLp1Route, address(this), now);
        }
