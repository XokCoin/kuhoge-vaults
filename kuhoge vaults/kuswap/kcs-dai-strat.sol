...
// File: contracts/BIFI/interfaces/pandai/IMasterChef.sol
...
interface IMasterChef {
    ...
    function pendingdai(uint256 _pid, address _user) external view returns (uint256);
...
// File: contracts/BIFI/strategies/dai/StrategydaiLP.sol
...
contract StrategydaiLP is StratManager, FeeManager, GasThrottler {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address constant public wbnb = address(0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521);
    address constant public dai = address(0xc9baa8cfdde8e328787e29b4b078abf2dadc2055);
    address public want;
    ...
    // Routes
    address[] public daiToWbnbRoute = [dai, wbnb];
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            daiToLp0Route = [dai, wbnb];
        } else if (lpToken0 != dai) {
            daiToLp0Route = [dai, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            daiToLp1Route = [dai, wbnb];
        } else if (lpToken1 != dai) {
            daiToLp1Route = [dai, wbnb, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(dai).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWbnb, 0, daiToWbnbRoute, address(this), now);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != dai) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(daiHalf, 0, daiToLp0Route, address(this), now);
        }

        if (lpToken1 != dai) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(daiHalf, 0, daiToLp1Route, address(this), now);
        }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(dai).safeApprove(unirouter, uint256(-1));
