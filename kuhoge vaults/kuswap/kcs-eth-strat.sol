...
// File: contracts/BIFI/interfaces/paneth/IMasterChef.sol
...
// File: contracts/BIFI/strategies/eth/StrategyethLP.sol
...
contract StrategyethLP is StratManager, FeeManager, GasThrottler {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address constant public wbnb = address(0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521);
    address constant public eth = address(0xf55af137a98607f7ed2efefa4cd2dfe70e4253b1);
    address public want;
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            ethToLp0Route = [eth, wbnb];
        } else if (lpToken0 != eth) {
            ethToLp0Route = [eth, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            ethToLp1Route = [eth, wbnb];
        } else if (lpToken1 != eth) {
            ethToLp1Route = [eth, wbnb, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(eth).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWbnb, 0, ethToWbnbRoute, address(this), now);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != eth) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(ethHalf, 0, ethToLp0Route, address(this), now);
        }

        if (lpToken1 != eth) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(ethHalf, 0, ethToLp1Route, address(this), now);
        }
