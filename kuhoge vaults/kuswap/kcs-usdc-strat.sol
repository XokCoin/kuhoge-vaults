...
// File: contracts/BIFI/interfaces/panusdc/IMasterChef.sol
...
// File: contracts/BIFI/strategies/usdc/StrategyusdcLP.sol
...
contract StrategyusdcLP is StratManager, FeeManager, GasThrottler {
    ...
    // Routes
    address[] public usdcToWbnbRoute = [usdc, wbnb];
    address[] public usdcToLp0Route;
    address[] public usdcToLp1Route;
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            usdcToLp0Route = [usdc, wbnb];
        } else if (lpToken0 != usdc) {
            usdcToLp0Route = [usdc, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            usdcToLp1Route = [usdc, wbnb];
        } else if (lpToken1 != usdc) {
            usdcToLp1Route = [usdc, wbnb, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(usdc).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWbnb, 0, usdcToWbnbRoute, address(this), now);
    ...
    function addLiquidity() internal {
        uint256 usdcHalf = IERC20(usdc).balanceOf(address(this)).div(2);

        if (lpToken0 != usdc) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(usdcHalf, 0, usdcToLp0Route, address(this), now);
        }

        if (lpToken1 != usdc) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(usdcHalf, 0, usdcToLp1Route, address(this), now);
        }
