...
// File: contracts/BIFI/interfaces/pankus/IMasterChef.sol
...
contract GasThrottler {
    ...
    address public gasprice = address(0xb1bb624F1e0479d58bB7454F626B773fcBBF40D1);
...
abstract contract FeeManager is StratManager {
    uint constant public STRATEGIST_FEE = 50;
    uint constant public MAX_FEE = 10000;
    uint constant public MAX_CALL_FEE = 500;
...
// File: contracts/BIFI/strategies/kus/StrategykuskwbnbLP.sol
...
contract StrategykuskwbnbLP is StratManager, FeeManager, GasThrottler {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address constant public wbnb = address(0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521);
    address constant public kwbnb = address(0x639a647fbe20b6c8ac19e48e2de44ea792c62c5c);
    address constant public kus = address(0x4A81704d8C16d9FB0d7f61B747D0B5a272badf14);
    address public want;
    address public lpToken0;
    address public lpToken1;

    // Third party contracts
    address constant public masterchef = address(0x0cc7fB3626C55Ce4EfF79045E8E7CB52434431D4);
    uint256 public poolId;

    // Routes
    address[] public kusToWbnbRoute = [kus, wbnb];
    address[] public kusToLp0Route;
    address[] public kusToLp1Route;
    ...
    constructor(
        ...
        if (lpToken0 == kwbnb) {
            kusToLp0Route = [kus, wbnb, kwbnb];
        } else if (lpToken0 != kus) {
            kusToLp0Route = [kus, wbnb, kwbnb, lpToken0];
        }

        if (lpToken1 == kwbnb) {
            kusToLp1Route = [kus, wbnb, kwbnb];
        } else if (lpToken1 != kus) {
            kusToLp1Route = [kus, wbnb, kwbnb, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(kus).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWbnb, 0, kusToWbnbRoute, address(this), now);
    ...
    function addLiquidity() internal {
        uint256 kusHalf = IERC20(kus).balanceOf(address(this)).div(2);

        if (lpToken0 != kus) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(kusHalf, 0, kusToLp0Route, address(this), now);
        }

        if (lpToken1 != kus) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(kusHalf, 0, kusToLp1Route, address(this), now);
        }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(kus).safeApprove(unirouter, uint256(-1));
    ...
    function _removeAllowances() internal {
        ...
        IERC20(kus).safeApprove(unirouter, 0);
        IERC20(lpToken0).safeApprove(unirouter, 0);
        IERC20(lpToken1).safeApprove(unirouter, 0);
    }
}