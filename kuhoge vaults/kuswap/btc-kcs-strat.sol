...
// File: contracts/BIFI/interfaces/panbtc/IMasterChef.sol
...
// File: contracts/BIFI/strategies/btc/StrategybtcLP.sol
...
contract StrategybtcLP is StratManager, FeeManager, GasThrottler {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address constant public wbnb = address(0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521);
    address constant public btc = address(0x218c3c3d49d0e7b37aff0d8bb079de36ae61a4c0);
    address public want;
    ...
    address[] public btcToLp0Route;
    address[] public btcToLp1Route;
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            btcToLp0Route = [btc, wbnb];
        } else if (lpToken0 != btc) {
            btcToLp0Route = [btc, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            btcToLp1Route = [btc, wbnb];
        } else if (lpToken1 != btc) {
            btcToLp1Route = [btc, wbnb, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(btc).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWbnb, 0, btcToWbnbRoute, address(this), now);
    ...
    function addLiquidity() internal {
        uint256 btcHalf = IERC20(btc).balanceOf(address(this)).div(2);

        if (lpToken0 != btc) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(btcHalf, 0, btcToLp0Route, address(this), now);
        }

        if (lpToken1 != btc) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(btcHalf, 0, btcToLp1Route, address(this), now);
        }
