...
// File: contracts/BIFI/interfaces/panusdt/IMasterChef.sol
...
// File: contracts/BIFI/strategies/usdt/StrategyusdtLP.sol
...
contract StrategyusdtLP is StratManager, FeeManager, GasThrottler {
    ...
    // Routes
    address[] public usdtToWbnbRoute = [usdt, wbnb];
    address[] public usdtToLp0Route;
    address[] public usdtToLp1Route;
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            usdtToLp0Route = [usdt, wbnb];
        } else if (lpToken0 != usdt) {
            usdtToLp0Route = [usdt, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            usdtToLp1Route = [usdt, wbnb];
        } else if (lpToken1 != usdt) {
            usdtToLp1Route = [usdt, wbnb, lpToken1];
        }
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(usdt).balanceOf(address(this)).mul(45).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWbnb, 0, usdtToWbnbRoute, address(this), now);
    ...
    function addLiquidity() internal {
        uint256 usdtHalf = IERC20(usdt).balanceOf(address(this)).div(2);

        if (lpToken0 != usdt) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(usdtHalf, 0, usdtToLp0Route, address(this), now);
        }

        if (lpToken1 != usdt) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(usdtHalf, 0, usdtToLp1Route, address(this), now);
        }
